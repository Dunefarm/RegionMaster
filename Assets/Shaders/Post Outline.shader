...
#include "UnityCG.cginc"
	...
	for (int j = 0; j<NumberOfIterations; j += 1)
	{
		...
		ColorIntensityInRadius += tex2D(
			...
			).r;// / NumberOfIterations; //Outcommented to get flat outline
	}
	...
	half4 outcolor = ColorIntensityInRadius*outlineColor *2 + (1 - ColorIntensityInRadius)*tex2D(_SceneTex, float2(i.uvs.x, i.uvs.y));
	return outcolor;
	}
